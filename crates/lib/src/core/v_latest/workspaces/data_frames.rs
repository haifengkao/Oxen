...
use crate::storage::version_store::{LocalFilePath, VersionLocation};
...
pub async fn index(workspace: &Workspace, path: &Path) -> Result<(), OxenError> {
    ...
    let mut _local_version_file: Option<LocalFilePath> = None;
    let mut _temp_file = None;
    let version_path = match version_store.version_location(&hash_str).await? {
        VersionLocation::Local(_) => {
            let local_path = version_store.get_version_path(&hash_str).await?;
            let path = local_path.to_pathbuf();
            _local_version_file = Some(local_path);
            path
        }
        VersionLocation::S3 { .. } => {
